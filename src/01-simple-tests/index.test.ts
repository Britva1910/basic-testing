...
describe('simpleCalculator tests', () => {
  test('should add two numbers', () => {
    ...
    expect(
      simpleCalculator({ a: 999999999999999, b: 1, action: Action.Add }),
  ...
  test('should subtract two numbers', () => {
    ...
    expect(simpleCalculator({ a: 100, b: 50, action: Action.Subtract })).toBe(
      50,
    );
    expect(simpleCalculator({ a: -100, b: -50, action: Action.Subtract })).toBe(
      -50,
  ...
  test('should multiply two numbers', () => {
    ...
    expect(simpleCalculator({ a: -5, b: 3, action: Action.Multiply })).toBe(
      -15,
    );
  ...
  test('should divide two numbers', () => {
    ...
    expect(simpleCalculator({ a: 100, b: 0.5, action: Action.Divide })).toBe(
      200,
    );
  ...
  test('should return null for invalid arguments', () => {
    ...
    expect(simpleCalculator({ a: 5, b: 'test', action: Action.Subtract })).toBe(
      null,
    );
    expect(
      simpleCalculator({ a: 'hello', b: 'world', action: Action.Multiply }),
    ).toBe(null);
    expect(simpleCalculator({ a: {}, b: [], action: Action.Divide })).toBe(
      null,
    );
    expect(
      simpleCalculator({ a: true, b: false, action: Action.Exponentiate }),
    ).toBe(null);
  });
});
